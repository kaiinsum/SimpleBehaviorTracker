        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                if mouth_expansion > 0.8:
                    print("Ngáp")
                    if credit>4:
                        credit-=1.001**(100-credit)*0.001*credit**2
                    ifngap = False
                else:
                    ifngap = True
                
                # eye alg
                left_gaze = detect_gaze_direction(landmarks, LEFT_EYE, LEFT_PUPIL)*lefteye_test
                right_gaze = detect_gaze_direction(landmarks, RIGHT_EYE, RIGHT_PUPIL)*righteye_test
                ...
                if avg_eye < 0.17 and ifchop1mat==False and left_eye < 0.08 and right_eye < 0.08:
                    #cv2.putText(combined_image, "BENH NHAN CHET ROI", (w + 20, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    clock += 1
